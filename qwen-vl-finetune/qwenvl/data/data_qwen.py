...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        sample_id = sources[0].get("id", f"index:{i}")
        data_path = sources[0].get(
            "data_path", self.list_data_dict[i].get("data_path", "")
        )
        #ipdb.set_trace()
            ...
            for thw in grid_thw:
                if (thw <= 0).any():
                    raise BadSampleError(
                        f"id={sample_id} image={image_file} bad image_grid_thw={thw}"
                    )
        ...
        if "video" in sources[0]:
            ...
            for thw in video_grid_thw:
                if (thw <= 0).any():
                    raise BadSampleError(
                        f"id={sample_id} video={video_file} bad video_grid_thw={thw}"
                    )
