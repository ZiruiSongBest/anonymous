...
class BadSampleError(RuntimeError):
    """Raised when a sample is invalid and should be skipped."""


def maybe_dump_bad_sample(sample, sample_id, data_path, seq_len, max_len):
    dump_id = os.environ.get("QWENVL_DUMP_BAD_SAMPLE_ID", "")
    if dump_id and sample_id != dump_id:
        return
    dump_path = os.environ.get("QWENVL_BAD_SAMPLE_PATH", "bad_samples.jsonl")
    record = {
        "id": sample_id,
        "data_path": data_path,
        "seq_len": seq_len,
    ...
    if os.environ.get("QWENVL_DUMP_BAD_SAMPLE_STDOUT") == "1":
...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        ...
        raise RuntimeError("No video backend available (decord/torchcodec missing).")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_final_retries):
            try:
                random_index = random.randrange(len(self.list_data_dict))
                ...
                print(
                    f"[Try random #{attempt_idx}] Failed to fetch sample {random_index}. Exception:",
                    e,
                )
                last_err = e
                continue
    ...
    def get_data(self, sources) -> Dict[str, torch.Tensor]:
        ...
        sample_id = sources[0].get("id", "unknown")
        data_path = sources[0].get("data_path", "")
            ...
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    ...
                    for path in image_paths:
                        if not os.path.exists(path):
                            raise BadSampleError(
                                f"id={sample_id} data_path={data_path} missing image={path}"
                            )
                    ...
                    if not os.path.exists(image_file):
                        raise BadSampleError(
                            f"id={sample_id} data_path={data_path} missing image={image_file}"
                        )
                ...
                image, grid_thw = self.process_image_unified(image_file)
                image = [image]
            image_list = list(image) if isinstance(image, Sequence) else [image]
            grid_thw_list = list(grid_thw) if isinstance(grid_thw, Sequence) else [grid_thw]
            for idx, thw in enumerate(grid_thw_list):
                thw_tensor = (
                    thw if isinstance(thw, torch.Tensor) else torch.tensor(thw)
                )
                if (thw_tensor <= 0).any():
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} image={image_paths[idx]} bad image_grid_thw={thw_tensor}"
                    )
                expected_patches = int(thw_tensor.prod().item())
                actual_patches = int(image_list[idx].shape[0])
                if actual_patches != expected_patches:
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} image={image_paths[idx]} grid_thw={thw_tensor.tolist()} patches={actual_patches}"
                    )
                merged_tokens = expected_patches // (
                    self.data_args.image_processor.merge_size**2
                )
                if merged_tokens <= 0:
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} image={image_paths[idx]} merged_tokens={merged_tokens}"
                    )
            ...
            if isinstance(video_file, List):
                if len(video_file) > 1:
                    ...
                    for path in video_paths:
                        if not os.path.exists(path):
                            raise BadSampleError(
                                f"id={sample_id} data_path={data_path} missing video={path}"
                            )
                    ...
                    if not os.path.exists(video_file):
                        raise BadSampleError(
                            f"id={sample_id} data_path={data_path} missing video={video_file}"
                        )
                ...
                video = [video]
            video_list = list(video) if isinstance(video, Sequence) else [video]
            video_grid_list = (
                list(video_grid_thw)
                if isinstance(video_grid_thw, Sequence)
                else [video_grid_thw]
            )
            for idx, thw in enumerate(video_grid_list):
                thw_tensor = (
                    thw if isinstance(thw, torch.Tensor) else torch.tensor(thw)
                )
                if (thw_tensor <= 0).any():
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} video={video_paths[idx]} bad video_grid_thw={thw_tensor}"
                    )
                expected_patches = int(thw_tensor.prod().item())
                actual_patches = int(video_list[idx].shape[0])
                if actual_patches != expected_patches:
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} video={video_paths[idx]} grid_thw={thw_tensor.tolist()} patches={actual_patches}"
                    )
                merged_tokens = expected_patches // (
                    self.data_args.image_processor.merge_size**2
                )
                if merged_tokens <= 0:
                    raise BadSampleError(
                        f"id={sample_id} data_path={data_path} video={video_paths[idx]} merged_tokens={merged_tokens}"
                    )
        ...
        if max_len is not None and max_len > 0:
            ...
            if seq_len > max_len:
                maybe_dump_bad_sample(
                    sources[0], sample_id, data_path, seq_len, max_len
                )
                raise BadSampleError(
                    f"id={sample_id} data_path={data_path} seq_len={seq_len} max_len={max_len}"
                )
        ...
        if isinstance(sources, list):
            ...
            if max_len is not None and max_len > 0:
                ...
                if total_len > max_len:
                    raise BadSampleError(
                        f"packed seq_len={total_len} max_len={max_len}"
                    )
            new_data_dict = {
                "input_ids": input_ids,
                "labels": labels,
