...
class FlexiCubes:
    ...
    def _compute_vd(self, voxelgrid_vertices, surf_cubes_fx8, surf_edges, scalar_field,
        ...
        beta_group = beta_group.to(ue_group.dtype)

        vd = torch.zeros((total_num_vd, 3), device=self.device, dtype=ue_group.dtype)
        beta_sum = torch.zeros((total_num_vd, 1), device=self.device, dtype=ue_group.dtype)
        ...
        if voxelgrid_colors is not None:
            ...
            vd_color = torch.zeros((total_num_vd, C), device=self.device, dtype=uc_group.dtype)
